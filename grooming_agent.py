...
from i18n import t as tr, get_language, get_language_instruction

try:
    from linear_sync import LinearSync
    LINEAR_SYNC_AVAILABLE = True
except ImportError:
    LINEAR_SYNC_AVAILABLE = False

try:
    from jira_sync import JiraSync
    JIRA_SYNC_AVAILABLE = True
except ImportError:
    JIRA_SYNC_AVAILABLE = False
...
class JiraClient:
    ...
    def get_backlog(self, project_key: str, max_results: int = 50) -> list[dict]:
        ...
        if not all([self.url, self.email, self.token]):
            print("⚠️  " + ("Jira not configured — skipping" if __import__("i18n").LANGUAGE == "en" else "Jira не настроена — пропускаем"))
            return []
...
class LinearClient:
    ...
    def get_projects(self) -> list[dict]:
        ...
        try:
            ...
            print(f"⚠️  Linear projects: {e}")
            return []

    def get_backlog(self, project_slug: str = None, max_results: int = 50) -> list[dict]:
        """Получить незаоценённые задачи из Linear."""
        if not self.token:
            print("⚠️  " + ("Linear not configured — skipping" if __import__("i18n").LANGUAGE == "en" else "Linear не настроен — пропускаем"))
            return []
...
ENRICH_PROMPT = """{lang_instruction}Ты опытный продакт-менеджер. Уточни описание задачи.
...
ESTIMATE_PROMPT = """{lang_instruction}Оцени сложность задачи по шкале Fibonacci с confidence score.
...
SPLIT_PROMPT = """{lang_instruction}Разбей большую задачу на подзадачи, каждая не более 5 story points.
...
ACCEPTANCE_PROMPT = """{lang_instruction}Напиши критерии приёмки для задачи в формате Given/When/Then.
...
PRIORITIZE_PROMPT = """{lang_instruction}Расставь приоритет задачи используя фреймворк RICE.
...
def log(state: GroomingState, step: str, data: dict) -> list:
    ...
    label = "task" if __import__("i18n").LANGUAGE == "en" else "задача"
    print(f"[{step.upper()}] {label} {entry['task_id']}")
    return state.get("audit", []) + [entry]

def call_model(prompt: str) -> str:
    # Подставляем языковую инструкцию
    prompt = prompt.replace("{lang_instruction}", get_language_instruction())
...
def load_tasks(state: GroomingState) -> GroomingState:
    ...
    if not linear_slug:
        ...
        if projects:
            ...
            choice = input(tr("select_all")).strip()
    ...
    if not jira_key:
        ...
        if projects:
            ...
            choice = input(tr("select_project")).strip()
            if choice.isdigit() and 0 < int(choice) <= len(projects):
    ...
    print("\n" + tr("loaded_tasks", jira=len(jira_tasks), linear=len(linear_tasks)))
    print(tr("unique_tasks", n=len(unique_tasks)))

    if not unique_tasks:
        print(tr("no_tasks_demo"))
        unique_tasks = DEMO_TASKS

    # Показываем список
    print("\n" + tr("tasks_for_grooming"))
...
def pick_next_task(state: GroomingState) -> GroomingState:
    ...
    task = tasks[idx]
    print(f"\n{'─'*60}")
    print(tr("task_header", i=idx+1, total=len(tasks), title=task['title']))
    print(tr("task_source", source=task['source'].upper(), status=task.get('status', '—')))
...
def enrich_description(state: GroomingState) -> GroomingState:
    ...
    result = parse_json(
        call_model(ENRICH_PROMPT.format(lang_instruction=get_language_instruction(), 
            title=task["title"],
...
def estimate(state: GroomingState) -> GroomingState:
    ...
    result = parse_json(
        call_model(ESTIMATE_PROMPT.format(lang_instruction=get_language_instruction(), 
            title=task["title"],
    ...
    print(tr("estimating", sp=sp, confidence=confidence))
...
def split_if_large(state: GroomingState) -> GroomingState:
    ...
    task = state["current_task"]
    print(tr("splitting", sp=state["story_points"]))

    result = parse_json(
        call_model(SPLIT_PROMPT.format(lang_instruction=get_language_instruction(), 
            title=task["title"],
            description=state["enriched_description"],
    ...
    if subtasks:
        print(tr("split_into", n=len(subtasks)))
        for st in subtasks:
...
def check_acceptance(state: GroomingState) -> GroomingState:
    ...
    result = parse_json(
        call_model(ACCEPTANCE_PROMPT.format(lang_instruction=get_language_instruction(), 
            title=task["title"],
            description=state["enriched_description"],
        )),
    ...
    print(tr("acceptance_done", n=len(criteria)))
...
def prioritize(state: GroomingState) -> GroomingState:
    ...
    result = parse_json(
        call_model(PRIORITIZE_PROMPT.format(lang_instruction=get_language_instruction(), 
            title=task["title"],
            description=state["enriched_description"],
            story_points=state["story_points"],
    ...
    print(tr("priority_set", priority=priority, score=rice))
...
def human_checkpoint(state: GroomingState) -> GroomingState:
    ...
    print(f"\n{'='*60}")
    print(tr("clarification_needed"))
    print(f"  {task['title']}")
    print(tr("clarification_question", q=state.get("human_feedback", "")))
    print(tr("current_estimate", sp=state["story_points"], confidence=state["confidence"]))
    print(f"\nВарианты:")
...
def save_groomed_task(state: GroomingState) -> GroomingState:
    ...
    # Синхронизируем с Linear
    if LINEAR_SYNC_AVAILABLE:
        try:
            sync = LinearSync()
            result = sync.upsert_task(groomed)
            if result["action"] in ("created", "updated"):
                groomed["linear_id"] = result["linear_id"]
        except Exception as e:
            print(f"   ⚠️  Linear sync: {e}")

    # Синхронизируем с Jira
    if JIRA_SYNC_AVAILABLE:
        try:
            jira_sync = JiraSync()
            jira_result = jira_sync.upsert_task(groomed)
            groomed["jira_synced"] = jira_result.get("action") == "updated"
        except Exception as e:
            print(f"   ⚠️  Jira sync: {e}")

    groomed_tasks = state.get("groomed_tasks", []) + [groomed]
    print("\n" + tr("task_done", title=task["title"]))
...
"""
    ...
    print(f"\n{'='*60}")
    print("\n" + tr("grooming_done"))
    print(tr("groomed_count", n=len(tasks)))
    print(f"   P0: {len(by_priority['P0'])} | P1: {len(by_priority['P1'])} | P2: {len(by_priority['P2'])}")
    print(tr("report_file", filename=filename))
    print(tr("audit_file", filename=audit_file))
